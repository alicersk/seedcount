...
source = pd.DataFrame((species,season,bcolor), index=None).T
source["bcolor"]=source["bcolor"].astype('category')

seasonaldistribution2 = alt.Chart(source).mark_bar(size=100).encode(
    x="season",
    y="species",
    color="bcolor"
).properties(width=500, height=300)

st.altair_chart(seasonaldistribution2)   