...
def format_plotdata(data, stats, Season):
    ...
    (seeddf.index.values, 
        ...
        seeddf["autumn"]),
            ...
            2: "plants_per_yard", 3:"ht", 4:"bloom_color", 5:"spring", 6:"summer", 7:"autumn"})
    ...
    if Season=='Winter':
        source.loc[source["Autumn"]==0, 'bloom_color'] = 'None'
        source['ht']*=.5
    else:    
...
def format_chartdata(data, stats, season):
    ...
    #duplicate rows so that each plant (per square yard desired density) is its own row
    blooming=forbs.loc[np.repeat(forbs.index.values, forbs.plants_per_yard)]
    #create counts by bloom color 
    df1=blooming.groupby([f'{season}','bloom_color']).count().reset_index().rename(columns={'plants_per_yard':f'{season.upper()}'}).filter([f'{season}', 'bloom_color', f'{season.upper()}'])
...
def seasonality_chart(data, stats):
    """
    This function plots number of stems in bloom per square yard in each season to assess aesthetic quality and floral resource abundance.
    """
    ...
    seasonaldistribution = alt.Chart(data=source, title='Bloom Density by Season').mark_bar(size=100).encode(
        ...
        y=alt.Y('value:Q',axis=alt.Axis(title='number of plants per square yard in bloom')),
        color=color
